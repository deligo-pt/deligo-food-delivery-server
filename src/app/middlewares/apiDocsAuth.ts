...
const checkBasicAuth = (req: Request, res: Response): boolean => {
  ...
  const unauthorized = () => {
    ...
    res.status(401).json({ success: false, message: 'AUTHENTICATION_REQUIRED' });
  };
...
export const apiDocsAuth = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const cookieDeviceId = req.cookies?.[DEVICE_COOKIE_NAME] as string | undefined;
...
export const listApiDocsDevices = catchAsync(async (_req: Request, res: Response) => {
  ...
  const devices = await Promise.all(
    keys.map(async (key) => {
      ...
      return { id: key.slice(DEVICE_KEY_PREFIX.length), ...device, expiresInSeconds };
    }),
  );
...
export const revokeApiDocsDevice = catchAsync(async (req: Request, res: Response) => {
  const deviceKey = `${DEVICE_KEY_PREFIX}${req.params.id}`;

  if (!(await RedisService.exists(deviceKey))) {
    res.status(httpStatus.NOT_FOUND).json({ success: false, message: 'DEVICE_NOT_FOUND' });
    return;
  }

  await RedisService.del(deviceKey);
  res.status(httpStatus.OK).json({ success: true, message: 'DEVICE_REVOKED' });
});
