...
const confirmIngredientOrder = async (
    ...
    const existingOrder = await IngredientOrder.findById(orderId).populate('orderDetails.ingredient');
    if (!existingOrder) {
    ...
    if (existingOrder.vendor.toString() !== currentUser._id.toString()) {
        throw new AppError(httpStatus.UNAUTHORIZED, 'Unauthorized to complete this order');
    }

    // 3. Prevent double processing
    if (existingOrder.paymentStatus === 'PAID') {
        throw new AppError(httpStatus.BAD_REQUEST, 'Order already paid and confirmed');
    }
    const ingredientData = existingOrder.orderDetails.ingredient as Partial<IIngredients>;
    ...
    const uniqueOrderId = `ING-ORD-${nanoid()}`;
