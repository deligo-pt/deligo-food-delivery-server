...
// create rating
const createRating = async (payload: TRating, currentUser: TAuthUser) => {
...
// get single rating
const getSingleRating = async (ratingId: string, currentUser: TAuthUser) => {
...
const getRatingSummary = async (currentUser: TAuthUser) => {
  const myProducts = await Product.find({
    vendorId: currentUser._id,
    isDeleted: false,
  }).select('_id');
